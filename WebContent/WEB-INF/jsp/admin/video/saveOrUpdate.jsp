...
<body>
	...
	<div class="container">
		...
		<c:if test="${not empty video }">
			<form class="form-horizontal" action="admin/video/edit.do"
				method="post">
		...
		<c:if test="${empty video }">
			<form class="form-horizontal" action="admin/video/add.do"
				method="post">
				...
				<div class="form-group">
					...
					<div class="col-sm-10">
						<input type="text" class="form-control" name="videoTitle"
							id="speakerName" value="${video.videoTitle }" placeholder="视频标题">
					</div>
				</div>
				<div class="form-group">
					...
					<div class="col-sm-10">
						<select name="speakerId" id="speakerId" class="form-control">
							...
							<c:forEach items="${speakers }" var="sp">
								<option value="${sp.id }"
									${video.speakerId==sp.id ? "selected" : "" }>${sp.speakerName }</option>
					...
					<div class="col-sm-10">
						<select name="courseId" id="courseId" class="form-control">
							...
							<c:forEach items="${courses }" var="c">
								<option value="${c.id }"
									${video.courseId==c.id ? "selected" : "" }>${c.courseName }</option>
